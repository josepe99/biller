...
export class SaleDatasource extends BaseDatasource<'sale'> {
  ...
  async getInvoicesWithFilters(
    ...
    if (Array.isArray(statuses) && statuses.length > 0) {
      const allowedStatuses = (Object.values(SaleStatus) as string[]).filter(Boolean)
      const normalizedStatuses = statuses
        .filter((status): status is string => typeof status === 'string' && allowedStatuses.includes(status))
    ...
    const min = typeof minTotal === 'number' && !Number.isNaN(minTotal) ? minTotal : undefined
    const max = typeof maxTotal === 'number' && !Number.isNaN(maxTotal) ? maxTotal : undefined
    ...
    const where: Prisma.SaleWhereInput = conditions.length > 1 ? { AND: conditions } : conditions[0]
